#include "sepia_serial.h"
#include <algorithm>
ImageBuffer SerialSepiaFilter::apply(const ImageBuffer& in, int) {
    ImageBuffer out = in.clone();
    const int W=in.width, H=in.height, C=in.channels;
    for (int r=0;r<H;++r) for (int c=0;c<W;++c) {
        float R,G,B;
        if (C>=3) {
            R=(float)in.at(r,c,0); G=(float)in.at(r,c,1); B=(float)in.at(r,c,2);
        } else {
            R=G=B=(float)in.at(r,c,0);
        }

...
__global__ void sharpenKernel(const uint8_t* __restrict__ in,
                               ...
                               int W, int H, int C)
{
    ...
    static const int K[3][3] = {{0,-1,0},{-1,5,-1},{0,-1,0}};
    ...
    for (int ch = 0; ch < C; ++ch) {
        ...
        out[(row*W+col)*C+ch] = clamp255((float)acc);
    }
}

ImageBuffer CudaSharpenFilter::apply(const ImageBuffer& input, int) {
    ...
    sharpenKernel<<<grid, block>>>(d_in, d_out,
